//! jp-civil-law-hard (民法、paraphrase + scenario) golden Q&A に対し、
//! **実 LLM (gemma-4-E4B GGUF)** + `LlmRewriter` / `MultiExpandRewriter` を回して
//! Run 35 (LlmRewriter@0.05) / Run 34 (MultiExpand@0.10) の **filter sweet spot
//! を別 corpus でも汎化できるか** を A/B 計測する (Run 38)。
//!
//! 動機: Run 35 / 37 までの結果は全て yokohama 1 corpus 由来。yokohama は
//! caption rerank が k=1 を支配する「lookup 寄り」コーパスで rewriter の利得が
//! 出にくかった。jp-civil-law-hard は **paraphrase / scenario 比 100%** で
//! rewriter が +21pp 効くと既知の (Run 8) コーパス。ここで filter sweet spot を
//! 取り直し、yokohama と「同じ閾値」になるか「別 corpus では違うか」を見る。
//!
//! 実行 (gemma-4-E4B GGUF + static-jp が必須):
//! ```sh
//! cargo run -p ellisii-sdk \
//!   --features static-jp,llamacpp \
//!   --example eval_civil_law_hard_llm --release
//! ```
//!
//! 結果は `docs/eval/recall-evals.md` Run 38 に追記する。
...
async fn run() -> anyhow::Result<()> {
    ...
    println!(
        "\n=== jp-civil-law-hard: filter sweep (gemma-4-E4B, Run 38) ===\n\
