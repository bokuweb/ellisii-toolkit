...
const Q_BY_CAPTION: &[(&str, &str)] = &[
    ...
    (
        "普通徴収の納税通知書はいつまでに納税者へ届くか",
        "普通徴収の方法による納税通知書",
    ),
    (
        "ふるさと納税で寄附金税額控除を受けるための申告は",
        "寄附金税額控除",
    ),
    ...
    (
        "新築の長期優良住宅は固定資産税が減額されますか",
        "認定長期優良住宅",
    ),
    (
        "課税ミスでもらいすぎた税金や取り損ねた税金の処理",
        "賦課もれ",
    ),
    ("都市計画税の納期は何回に分かれているか", "都市計画税の納期"),
    (
        "市税徴収のための条例の施行に必要な事項は誰が定めるか",
        "委任",
    ),
    (
        "市税の課税標準額の端数はどう計算しますか",
        "課税標準額、税額等の端数計算",
    ),
    (
        "土地の価格がいくら以下なら固定資産税はかからない",
        "固定資産税の免税点",
    ),
